...
if ! has_marker "node-config"; then
  WARNINGS+="Before configuring this node, invoke the n8n skill (Skill tool) and read references/workflow-nodes.md. Configuration is operation-aware: required fields depend on the resource/operation pair, and displayOptions control which fields are even visible. Never assume parameter names; confirm them against the get_node definition you're about to fetch. For interpreting any validation results afterward, references/workflow-validate.md."
...
[Code node detected]
Invoke the references/workflow-code.md skill before writing a Code node. Decision order: expression first, then an arrow function inside Edit Fields, then a Code node only if neither can do the job. Default to JavaScript; use Python only when the user explicitly asked; code an AI agent will call is a different runtime (the Code Tool) — that route covers all three. Much of what looks like a Code-node job has a native node instead (Crypto, XML, HTTP Request pagination) or belongs in an expression — the skill covers which, so route there rather than reaching for Code by reflex."
fi
...
[Merge node detected]
STOP and invoke the n8n skill (Skill tool) and read references/workflow-nodes.md. Two silent failure modes:
...
2. The input index pairing is easy to wire backwards; off-by-one passes data from the wrong branch with no error. For wiring mistakes that survive into runtime, references/workflow-validate.md."
fi
...
STOP and invoke the n8n skill (Skill tool), then read references/workflow-code.md (loop patterns) and references/workflow-design.md (batch architecture).
...
Invoke the references/workflow-expressions.md skill first. Date math, formatting, and parsing usually work inline with Luxon at the consumer field, with no separate node:
...
STOP and invoke the n8n skill (Skill tool) and read references/workflow-nodes.md and references/workflow-design.md. Gotchas that catch people:
...
For the n8n_manage_datatable tool itself (CRUD, filtering, dry-run), see references/workflow-instances.md."
fi
