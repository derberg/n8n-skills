...
"Before a credential operation: invoke the n8n skill (Skill tool) and read references/workflow-instances.md. Credentials hold live secrets — use the credential system and getSchema, never inline tokens into text fields. If this account has multiple n8n instances (the n8n_instances tool is present), read that reference's multi-instance section too: a credential create/update/delete routes to the currently-selected instance. The server fail-closes only the AMBIGUOUS case with INSTANCE_AMBIGUOUS (this session never switched; switch on this session, then retry) — an explicit switch to the wrong instance still writes the secret there, so verify current (n8n_instances list) immediately before the write."
