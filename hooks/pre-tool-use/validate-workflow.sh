...
"Before validating, run the antipattern scan node-by-node (invoke the n8n skill (Skill tool) and read references/workflow-validate.md and references/workflow-design.md): Set nodes feeding only 1 consumer should be inlined; Code nodes doing pure field shaping should be Edit Fields with arrow functions; Merges with 3+ wires need numberOfInputs set explicitly; \$json.x in branchy workflows should be \$('Node').item.json.x; DateTime nodes should be Luxon expressions. validate_workflow does not catch any of these; only the manual scan does. Validation passing means the JSON is well-formed, not that the workflow is correct."
