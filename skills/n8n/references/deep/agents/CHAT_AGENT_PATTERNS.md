...
Passing only the array fails **silently** — the Slack node accepts the input, the message posts with no rich content, and there's no error or warning. → **workflow-nodes** `NODE_FAMILY_GOTCHAS.md` (Slack section).
...
- `onError: 'continueErrorOutput'` and error UX → **workflow-errors**
- Slack node parameter shapes (Block Kit) → **workflow-nodes** `NODE_FAMILY_GOTCHAS.md` (Slack section)
- Receiving uploaded files / returning generated files per surface → **workflow-binary**
