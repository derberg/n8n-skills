...
- **`Call Agent core`** has `onError: 'continueErrorOutput'`, so `main[1]` carries the error branch (→ **workflow-errors**). The loading reaction is removed on the success path; the error branch surfaces a link instead of hanging forever.
...
- Error branch on the core call → **workflow-errors**
