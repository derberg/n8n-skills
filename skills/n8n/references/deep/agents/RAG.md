...
- **Database or Data Table for exact lookups.** "Look up customer X's record", "fetch issue #1234", "get rows where status = 'open'" are NOT RAG problems — use a query directly. → **workflow-nodes** for DB nodes.
...
- Data Tables as an alternative to a vector store for small structured data → **workflow-nodes**
