...
The fix is to keep the binary on a branch that doesn't touch it, and recombine. This is the same Merge node covered in **workflow-nodes**'s gotchas; here it's used specifically to re-attach binary.
...
Two wiring details that bite (both detailed in **workflow-nodes**'s Merge section):
