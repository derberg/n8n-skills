...
- You avoid the classic SplitInBatches gotchas (see **workflow-code**) inside a workflow that's supposed to be a clean function.
...
The pattern generalizes: any time a capability has both a pull path (look up by ID) and a push path (caller already holds the data, including binary or a template), the split applies. For the binary-handling specifics, see **workflow-binary**; for wiring the typed outer as an agent tool, **workflow-agents**.
...
The catch: the caller doesn't know when — or whether — any of them finished. So this only works with a **separate completion-tracking mechanism**, typically a Data Table the sub-workflow writes to as it progresses (manage it with `n8n_manage_datatable` — see **workflow-instances**).
...
Pair the per-job error handling (the row's `error` status) with **workflow-errors** so a failed job is recorded, not just silently absent.
