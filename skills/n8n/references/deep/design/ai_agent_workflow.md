...
> **For agent design depth, use the `workflow-agents` route.** This file covers where an AI agent sits in a workflow's architecture (trigger → agent → output, the `ai_*` sub-node connection types). The `workflow-agents` route owns the design rules: tool selection and `$fromAI` parameters, the system-prompt vs tool-description split, structured output with autoFix, memory and sessionId, human-in-the-loop review, RAG, and chat shell/core/sub-agent topologies. Start there when building or debugging an agent.
...
For the deep slot mechanics — tool types, `$fromAI` parameters, memory configuration, parser schemas — see **workflow-agents**.
...
For the *content* of these (tool descriptions, system prompts, schema design), see **workflow-agents** `EXAMPLES.md`.

---

## What the deep design lives in workflow-agents

This file is the workflow-architecture view. The design depth below is owned by **workflow-agents** — go there, don't duplicate it here:

- **Tool configuration** (the four tool types, native vs `.toolWorkflow` vs HTTP Request Tool vs MCP Client, `$fromAI()` anatomy, tool names/descriptions as prompt) → **workflow-agents** `TOOLS.md`, and `SUBWORKFLOW_AS_TOOL.md` for wiring a sub-workflow as a tool.
- **Memory configuration** (buffer/window/postgres/redis, `contextWindowLength`, sessionId handling per trigger) → **workflow-agents** `MEMORY.md`.
- **Agent vs chain vs classifier choice, prompt engineering, system-prompt vs tool-description split** → **workflow-agents** `SYSTEM_PROMPT.md` (and the SKILL.md "Pick the right node" table).
- **RAG chains, structured output, streaming, fallback models** → **workflow-agents** `RAG.md` and `STRUCTURED_OUTPUT.md`.
- **Human review / gating destructive tools** → **workflow-agents** `HUMAN_REVIEW.md`.
- **Error handling** (tool failures, LLM API errors, retries, error workflows) → **workflow-errors**, plus the agent-specific notes in **workflow-agents**.
- **Performance, security, testing, common gotchas** → **workflow-agents** (anti-patterns table and quick-reference checklist) for the agent-specific ones; the workflow lifecycle (test → validate → activate) is in this skill's SKILL.md "Workflow lifecycle" section.

One workflow-architecture safety note worth restating here: **any tool that fetches third-party content** (HTTP Request, web search, MCP Client, scrapers) can return attacker-controlled text that reaches the agent's context — indirect prompt injection. If the agent can both *read the internet* AND *take an action the user can't undo*, put a guardrail (human review, read-only scopes) between them. The detail lives in **workflow-agents** `HUMAN_REVIEW.md` and the **workflow-agents** anti-patterns.

---

## Checklist for AI Agent Workflows

Architecture-level checks (the design-level checklist lives in **workflow-agents**):
...
5. For all design depth — tools, memory, prompts, structured output, RAG, human review, chat topologies — go to **workflow-agents**.

**Pattern**: Trigger → AI Agent (Model + Tools + Memory + optional Parser) → Output

**Related**:
- **workflow-agents** — the deep agent design guide (tools, memory, prompts, structured output, RAG, human review, chat topologies)
...
- **workflow-errors** — tool-failure and LLM-error handling
