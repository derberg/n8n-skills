...
Set `responseMode: "responseNode"` on the Webhook trigger — without it the trigger acknowledges immediately (`onReceived`) and the caller never sees your computed response. (See **workflow-nodes** NODE_FAMILY_GOTCHAS.md for the Webhook/Respond traps.)
